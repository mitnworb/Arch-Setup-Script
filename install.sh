...
pacstrap /mnt base ${kernel} ${microcode} linux-firmware base-devel grub grub-btrfs snapper wget snap-pac efibootmgr sudo vim fortune-mod git networkmanager apparmor python-psutil gdm gnome gnome-tweaks gnome-themes-extra pipewire-pulse pipewire-alsa pipewire-jack neofetch yadm flatpak firewalld zram-generator adobe-source-han-sans-otc-fonts adobe-source-han-serif-otc-fonts gnu-free-fonts reflector mlocate man-db
...
sed -i 's/# %wheel ALL=(ALL:ALL) ALL/%wheel ALL=(ALL) NOPASSWD:ALL/g' /mnt/etc/sudoers
